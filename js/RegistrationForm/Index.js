...
$('#btnRegister').click(e => {
  ...
  $('#addTechinicalTable > tbody > tr').each(function (i) {
    addTechinical.push({
      id: $(`tr#row${i} > td > span`).val(),
      name: $(`tr#row${i} > td > input`).val(),
      followType: $(`tr#row${i} > td > select`).val()
    })
  });
  ...
  flag && func.postDataForm(dataForm, addTechinical, resp => {
    if (resp.status) {
